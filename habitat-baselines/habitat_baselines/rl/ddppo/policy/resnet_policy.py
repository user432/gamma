...
from habitat.tasks.rearrange.rearrange_sensors import GraspSensor
...
from habitat_baselines.rl.ddppo.policy.pointnet import PointNetEncoder
...
class ResNetEncoder(nn.Module):
    ...
    def forward(self, observations: Dict[str, torch.Tensor]) -> torch.Tensor:  # type: ignore
        ...
        # print("self.visual_keys", self.visual_keys)
...
class PointNavResNetNet(Net):
    ...
    def __init__(
        ...
        if GraspSensor.cls_uuid in observation_space.spaces:
            input_grasp_dim = 4 # incermental node, edge, total node, edge
            self.grasp_embedding = nn.Linear(input_grasp_dim, 32)
            rnn_input_size += 32
        ...
        self.grasp_encoder = PointNetEncoder(global_feat=True,  channel = 8)
        self.grasp_state_size = 512
        ...
        self.state_encoder = build_rnn_state_encoder(
            (0 if self.is_blind else self._hidden_size) + rnn_input_size + self.grasp_state_size,
            self._hidden_size,
            rnn_type=rnn_type,
    ...
    def forward(
        ...
        # ================ new add ========================
        if GraspSensor.cls_uuid in observations:

            x.append(self.grasp_embedding((observations["grasp_sensor"][:,64,2:6]).float()))
        ...
        grasp_state = self.grasp_encoder(grasp_k.float())

        x.append(grasp_state)
